...
const ScreenMess = () => {
    ...
    const loadChat = async () => {
        const url = `${API_CONFIG.BASE_URL}/api/v1/chat/find/${user.id}`;
        const data = await GetData(url);
        if (data !== 'Something went wrong') {
            if (Array.isArray(data)) {
                const dataSort = data.sort((a, b) => {
                    const dateA = a.latestMessage[0]
                        ? new Date(a.latestMessage[0].createdAt)
                        : new Date(0);
                    const dateB = b.latestMessage[0]
                        ? new Date(b.latestMessage[0].createdAt)
                        : new Date(0);
                    return dateB - dateA;
                });
    ...
    useEffect(() => {
        if (searchText !== prevSearchText.current) {
            if (searchText.trim().length > 0) {
                ...
                setChatAppear(
                    chatRooms.filter(function (chat) {
                        let num = 0;
                        if (chat.chatInfo.members[0].id == user.id) {
                            num = 1;
                        }
                        return chat.chatInfo.members[num].username
                            .toLowerCase()
            ...
            <TouchableOpacity
                onPress={() =>
                    navigation.navigate('chatRoom/room', {
                        chatId: item.chatInfo._id,
                        chat: item.chatInfo,
                        name: item.chatInfo.members[num].username,
                        idFriend: item.chatInfo.members[num].id,
                    })
                }
            >
                <CardMessage
                    ...
                    Status={
                        onlineUsers?.some(
                            (onlineUser) => onlineUser.userID === item.chatInfo.members[num]._id
                        )
                            ? 'online'
                            : 'offline'
                    }
