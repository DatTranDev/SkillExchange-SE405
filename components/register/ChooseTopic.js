...
class ChooseTopic extends React.Component {
    ...
    selectTopic = (item) => {
        this.setState((prevState) => ({
            learnTopic: prevState.learnTopic.map((learnTopic) =>
                learnTopic._id === item._id
                    ? { ...learnTopic, chosen: !learnTopic.chosen }
                    : learnTopic
            ),
        }));
    };
    ...
    fetchTopic = async () => {
        ...
        try {
            const response = await fetch(
                `${API_CONFIG.BASE_URL}/api/v1/topic/pagination?page=${this.state.page}&limit=8`
            );
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const json = await response.json();
            const topicsWithChosen = json.data.map((learnTopic) => ({
                ...learnTopic,
                chosen: false,
            }));
            this.setState((prevState) => ({
    ...
    render() {
        ...
        const topicID = this.state.learnTopic
            .filter((learnTopic) => learnTopic.chosen)
            .map((learnTopic) => learnTopic._id);
        params = {
            name: passing.name,
            image: passing.image,
