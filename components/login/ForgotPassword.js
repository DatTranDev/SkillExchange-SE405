...
class ForgotPassword extends React.Component {
                ...
                if (this.state.newPassword.length < 8) {
                    ...
                    try {
                        const response = await fetch(
                            `${API_CONFIG.BASE_URL}/api/v1/user/changepassword`,
                            {
                                method: 'PATCH',
                                headers: {
    ...
    sendMail = async () => {
        if (this.state.email == null || this.state.email == '') {
            ...
            this.setState({ isLoading: true });
            try {
                const response = await fetch(`${API_CONFIG.BASE_URL}/api/v1/service/sendEmail`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    render() {
        return (
            <GradienLayout innerStyle={this.state.verified ? { height: scale(500) } : {}}>
                ...
                <Text style={[styles.text_center, { color: COLORS.orange, marginBottom: 20 }]}>
                    Forgot Password
                </Text>
                <Image
                    ...
                    style={[
                        styles.image,
                        { height: scale(120), width: scale(120), marginVertical: 15 },
                    ]}
                />
                ...
                {/* Varify code */}
                {this.state.sended && (
                    <Text style={{ marginHorizontal: 20, fontFamily: 'Coda-Regular' }}>
                        Email: {this.state.email}
                    </Text>
                )}
                {this.state.sended && (
                ...
                {this.state.verified && (
                    <View>
                        ...
                        />
                        <Modal transparent={true} visible={this.state.showMessage}>
                            <Notification
