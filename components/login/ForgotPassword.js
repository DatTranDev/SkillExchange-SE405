...
class ForgotPassword extends React.Component {
        ...
        if (this.state.newPassword === null || this.state.newPassword === '') {
            ...
            if (this.state.newPassword !== this.state.confirmPassword) {
                ...
                if (this.state.newPassword.length < 8) {
                    ...
                    try {
                        const response = await fetch('https://se405-skillexchangebe.onrender.com/api/v1/user/changepassword', {
                            method: 'PATCH',
                            headers: {
    ...
    sendMail = async () => {
        if (this.state.email == null || this.state.email == '') {
            ...
            this.setState({ isLoading: true });
            try {
                const response = await fetch('https://se405-skillexchangebe.onrender.com/api/v1/service/sendEmail', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
            ...
            <GradienLayout innerStyle={this.state.verified ? { height: scale(500) } : {}}>
                ...
                {!this.state.sended && !this.state.verified &&
                    <View>
                        ...
                        }}>We will send you an email to reset your password</Text>
